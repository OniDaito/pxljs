  ...
  _traverse : (base_node) ->
    ...
    if base_node.spotLights.length > 0
      ...
      for light in base_node.spotLights
        if light.shadowmap
          @uber_defines.push "ADVANCED_CAMERA" if "ADVANCED_CAMERA" not in @uber_defines
          @uber_defines.push "SHADOWMAP" if "SHADOWMAP" not in @uber_defines
          @uber_defines.push "PACKING" if "PACKING" not in @uber_defines
